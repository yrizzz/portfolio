...
async function main() {
  console.log('🚀 Starting Clean Migration: MySQL → MongoDB\n');
  ...
  try {
    ...
    await Models.SocialMedia.deleteMany({});
    await Models.SiteConfig.deleteMany({});
    await Models.ApiEndpoint.deleteMany({});
    ...
    console.log('   2. Update routes: npm run migrate:update-routes');
  } catch (error) {
